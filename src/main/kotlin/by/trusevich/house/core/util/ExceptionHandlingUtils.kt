...
fun toResponse(ex: Exception, status: HttpStatus) = toResponse(ex, ex.message, status)

fun toValidationResponse(ex: MethodArgumentNotValidException): ResponseEntity<Any> =
    ResponseEntity.status(UNPROCESSABLE_ENTITY).contentType(APPLICATION_JSON_UTF8).body(toResponse(ex,
        VALIDATION_REASON,
